...
number of position. When you play the slide first, B<Sprite> will
stand in a default position. Just drag it where you wan't it to be
with button 1. Do this for each position. Then save it. It will
remenber where you have I<mouse-ly> specified each position.
The script example  is trivial :
...
add your speach added value to it. That why, I like to add to my
presentation, a documentation to each slides. This helps also to
prepare the presentation.

To add documentation to my slides, I have choosen to use LaTeX as a
language. Maybe in a future release, somebody else will want to
express attach documentation to slide in a différent language. 

So, that's the reason for the method C<latex> on a
C<Tk::SlideShow::Diapo> object.

To let you add an header and a footer of to your presentation paper, I
have added 2 methods for the C<Tk::SlideShow> object : C<latexheader> and C<latexfooter>.

To output the whole documentation on a file, you may use the method
C<latex> on the C<Tk::SlideShow> object (note that this is not on the
C<Tk::SlideShow::Diapo> object this time).
...
C<Tk::SlideShow> script :
	...
	$d->latex('blah blah on the menu slide');
	$d = $p->add('introduction', sub { ...; $p->load });
	$d->latex('blah blah on the introduction slide');
	...
	$d = $p->add('development',  sub { ...; $p->load });
	$d->latex('blah blah on the development slide');
	$d = $p->add('conclusion',   sub { ...; $p->load });
	$d->latex('blah blah on the conclusion slide');
...
=head2 Base Level
  ...
  $d->latex('..');		# add latex documentation
  my ($mw,$c,$h,$w) = ($p->mw,$p->can,$p->h,$p->w);
  ...
  $p->latex('toto.tex')	# produces on complete latex documentation
  $p->latexheader(?header?);
  $p->latexfooter(?footer?);
