...
Tk::SlideShow -- a B<Tk> B<Perl> module for building presentation.
...
build interactive and visual presentation with B<Perl> and B<Tk>,
...
build structured presentation, precisely computed,

=item *

take advantage of your knowledge of B<Perl> and B<Tk>,
...
build presentation that will let the attendees wonder how they will do
it with Microsoft PP C<:-)>.

=item *

build presentation without leaving your prefered  developping enviroment : Unix,
...
In this document, I have tried to entralace tutorial and full
description approach. This makes it a little bit surprising. But I
think it was possible in this case.

=head1 FIRST CONTACT

To build a presentation with B<Tk::SlideShow>, you had better to create a
directory for keeping all the files that will be created.
...
presentation.  It understand 2 parameters as the sizes (resp. width
and height) of the main windows. Without these parameters the sizes of
...
presentations. So you will develop it for, e.g. a definition of
C<1024x768> instead of C<1600x1280> desktop 21 inch screen. So, you
will begin your script with :

	my $p = Tk::SlideShow->init(1024,768);

As Larry says often, it's useless to C<or die>. Method C<init> return
an instance of B<Tk::SlideShow>. You may feel it useless to use an
instance of B<Tk::SlideShow> that will be probably uniq during the
...
lines are at the beginning of your script without mentioning it.
...
While running, note that you are able to drag the text "Hello World !"
with the button 1, where you want it to be.  Pressing key C<s>, you
will save its positions. Pressing key C<q> will quit the presentation. If
you play it again, you will find your text at the same place were you
let it just after you press key C<s>. Pressing key C<space> or C<backspace>
let's you navigate thru different slides. Here, you just have one
.. so ...

This is how it looks. Now let's explain what it means :

First I call the C<save> method. As decribe more precisely later, this make
all your slides sensible to a keypress on C<s> for  saving the artistic 
...
It's sounds simple, doesn't it ?

I am sure you have a tone of questions, for how to do this or
...
You have to give it a uniq name as a first parameter.  It's let you
reference it more easely in the future (as you will see
later). This name can be whatever a filename can be. Keep in mind,
that the position of most of your objects will be saved in file call
C<slide-name.pl>, where C<name> will be replace by the first argument
to method C<add>.

Then you have to give it a sub reference. This sub will receive the
slide as an argument, but you will probably never use it.
...
The method C<save> may be called, but only once. It just indicates to
B<Tk::SlideShow> that you do want B<Tk::SlideShow> to save the
modification you have done during the presentation pressing key
C<s>. If this is not what you want just comment the C<$p-E<gt>save;>
line.
...
an argument to method C<load>, but most of the time this is useless.

There is (at least) one file per slide that contains positions and
other characteristics of objects. The file will have the name
C<slide-xxx> where xxx is the name of the slide (that is the name you
give as a first argument to method C<add>). This file contains a
B<Perl> script that is automaticaly generated by B<Tk::SlideShow>. So
you will have at least as many files as you have slides. That's a good
reason, I think, for creating a specific directory for your
presentation.
...
initialized previously as I said aboce. It contains the canvas where
all objects will be drawn. That the second and the last time I recall
it to you.

In this case, this is clear that you like chocolate ... as a color for
...
To summarize what you have learned up to now, here is the look of the
...
Now you would be able to build any presentation. But it would be to
difficult to specify everything. So there are some more concept in
B<Tk::SlideShow> that will help you to define it.
...
it, since I already mention it before several times.
...
the file C<Tk/SlideShow.pm> and think of it as custumable examples.
...
to show source example in your show.  You will probably want to have a
B<Sprite> specialized representing these scripts examples. To be
consistant, you want it to look very similar in the whole presentation
slides. Even perhaps in all the presentations.
...
tex of the example which is passed as an argument to the function.
...
Often, you would like to explain progressively a complex slide. So,
you would like to let B<Sprites> appear in a particular order, so that
attendees will discover a complex slide progresively.

B<Tk::SlideShow> provide you with the ability to let the B<Sprites> appear
...
the items arriving on my slide, slipping for the let side of it to
there right place. You may have used as well method C<a_right> or
...
You can also change the attach (cardinal) point of the link on each
B<Sprite> it links together, by clicking button 1 or 3 on the link.
...
to 150. The family used by default is C<Utopia>. You may change this
...
So, you may write this small sub :
  ...
  sub small_summarize {
    my $count = 0;
    my @p = @plan;
    my ($fsmall,$fbig) = ($p->0_5, $p->f1);
    while(@p) {
      my ($slide,$title) = (shift @p, shift @p);
      my $id = "som/i$count";
      my ($color,$font) = $p->currentName eq $slide ?
	     ('red', $fbig) : ('blue',$fsmall) ;
      $p->Text($id,$title, -font, $font, -fill,$color, -anchor,'w');
...
here is a summurized synopsis of B<Tk::SlideShow> methods.

=head2 Base Level

  use Tk::SlideShow;
  my $p = Tk::SlideShow->new;	# initialize canvas according to the curretn screen
  $p->bg(?sub|color?);  # change background 
  $p->add(?'nom',sub);  # add a slide
  $p->clean;	        # clean the current slide
  my ($mw,$c,$h,$w) = ($p->mw,$p->can,$p->h,$p->w);
		        # retrieve key context variables 
...
=head2 Higher level
  ...
  $p->title_ne("Olivier Bouteille");
			# set a title to the slide (nord-est)
  $p->title_se("ATPS (At The Perl Service) !");
			# idem but for south-est

=head2 C<Sprites>

  $p->newSprite($id);	 # create an empty sprite
  $p->Text($id,$texte,@texte_options);
			 # Predefined Text sprite
  $p->Window($id,$widget,$canvas_widget_options);
			 # Predefined tk window Sprite
  $p->Image($filename);  # redefined Image Sprite
  $p->framed($id,$text); # Predefined frame text Srite
...
# Local Variables: ***
# mode: perl 
# LocalWords:  play shift add sub End load Tk::SlideShow new my save die can current mw
# LocalWords:  prompt-unix coderef sprite perl Tkperl createText chocolate bg
# LocalWords:  cliquera warp NAME
# End: ***
